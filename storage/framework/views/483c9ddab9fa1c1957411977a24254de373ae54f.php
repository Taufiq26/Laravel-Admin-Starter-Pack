    ...
    $( document ).ready(function() {
		...
		$('#form').submit(function(event) {
			...
			if($('#postType').val() == 'create'){
				url = "<?php echo e(url('api/roles/create')); ?>";
			}else{
                url = "<?php echo e(url('api/roles/update')); ?>/"+_selectedID;
			}
			...
			.done(function(res) {
                ...
                autoReload('#dataTableRoles', '<?php echo e(url("api/roles")); ?>')
		...
		$('#dataTableRoles tbody').on('click','button[name="btn_edit"]',function(event){
			...
			$.ajax({
				url: "<?php echo e(url('api/roles')); ?>/"+_selectedID,
				type: 'GET',
				dataType: 'json',
			})
			.done(function(res) {
		...
		$('#dataTableRoles tbody').on('click', 'button[name="btn_hapus"]', function(event) {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: "<?php echo e(url('api/roles/delete')); ?>/"+_selectedID,
                        type: 'DELETE',
                        dataType: 'json',
                    })
                    .done(function(res) {
		...
		$('#dataTableRoles tbody').on('click', 'button[name="btn_access"]', function(event) {
			...
			window.location.href = "<?php echo e(url('users-management/roles-access')); ?>/"+_selectedID;
		});
...
</script><?php /**PATH /Applications/MAMP/htdocs/laravel-admin-starter-pack/resources/views/services/roles/roles.blade.php ENDPATH**/ ?>