    ...
    $( document ).ready(function() {
		...
		$('#form').submit(function(event) {
			...
			if($('#postType').val() == 'create'){
				url = "<?php echo e(url('api/menus-access/create')); ?>";
			}else{
                url = "<?php echo e(url('api/menus-access/update')); ?>/"+_selectedID;
			}
			...
			.done(function(res) {
                ...
                // autoReload('#dataTableMenus', '<?php echo e(url("api/menus-access")); ?>')
                $('#refresh').click()
		...
		$('#dataTableMenus tbody').on('click','button[name="btn_edit"]',function(event){
			...
			$.ajax({
				url: "<?php echo e(url('api/menus-access')); ?>/"+_selectedID,
				type: 'GET',
				dataType: 'json',
			})
			.done(function(res) {
		...
		$('#dataTableMenus tbody').on('click', 'button[name="btn_hapus"]', function(event) {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: "<?php echo e(url('api/menus-access/delete')); ?>/"+_selectedID,
                        type: 'DELETE',
                        dataType: 'json',
                    })
                    .done(function(res) {
        ...
        function setDataTable(role_id){
            $('#dataTableMenus').DataTable({
                "bLengthChange":true,
                "pageLength":10,
                "ajax" : "<?php echo e(url('api/menus-access/by-role')); ?>/" + role_id,
                "columns": 
                [
                { 
                    targets:[0], 
...
</script><?php /**PATH /Applications/MAMP/htdocs/laravel-admin-starter-pack/resources/views/services/menu-access/menu-access.blade.php ENDPATH**/ ?>